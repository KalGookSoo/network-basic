...
**정답**: 클래스풀 주소 체계에서는 모든 IP 주소의 네트워크 부분과 호스트 부분이 동일한 비트 수를 가진다
...
**해설**: IP 프로토콜은 신뢰성 있는 데이터 전송을 보장하지 않습니다. IP는 최선형 전달(best-effort delivery) 방식을 사용하며, 패킷 손실, 순서 변경, 중복 전송 등이 발생할 수 있습니다. IP는 패킷 전달을 보장하지 않으며, 오류 제어나 흐름 제어 기능이 없습니다. 이러한 신뢰성 관련 기능은 상위 계층인 전송 계층(TCP)에서 제공됩니다. IP는 비연결형 통신을 제공하고, 각 패킷이 독립적으로 처리되며, 패킷 순서가 바뀔 수 있다는 특성은 모두 옳습니다.
...
#### 문제 1
**문제**: TCP의 특징으로 옳지 않은 것은?

**정답**: 브로드캐스트와 멀티캐스트를 지원한다

**해설**: TCP는 브로드캐스트와 멀티캐스트를 지원하지 않습니다. TCP는 연결 지향적 프로토콜로, 두 호스트 간의 1:1 통신(유니캐스트)만 지원합니다. 브로드캐스트(1:모두 통신)와 멀티캐스트(1:그룹 통신)는 UDP와 같은 비연결형 프로토콜에서 지원됩니다. TCP는 연결 지향적 프로토콜로 데이터 전송 전에 3-way 핸드셰이크를 통해 연결을 설정하고, 데이터의 순서를 보장하며, 흐름 제어와 혼잡 제어 기능을 제공합니다. 이러한 특성들은 신뢰성 있는 데이터 전송을 가능하게 하지만, 브로드캐스트나 멀티캐스트와 같은 다중 수신자 통신 방식과는 호환되지 않습니다.
...
**해설**: UDP(User Datagram Protocol) 헤더의 크기는 8바이트입니다. UDP 헤더는 다음 네 가지 필드로 구성됩니다: 출발지 포트(2바이트), 목적지 포트(2바이트), 길이(2바이트), 체크섬(2바이트). 이 단순한 구조는 UDP의 낮은 오버헤드와 빠른 처리 속도에 기여합니다. 반면, TCP 헤더는 기본적으로 20바이트(옵션 필드 제외)이며, 최대 60바이트까지 확장될 수 있습니다. UDP의 단순한 헤더 구조는 실시간 애플리케이션이나 빠른 데이터 전송이 중요한 경우에 유리하지만, 신뢰성과 흐름 제어 기능이 없다는 단점이 있습니다.

#### 문제 3
**문제**: TCP 연결 설정 과정인 3-way 핸드셰이크에 대한 설명으로 옳은 것은?

**정답**: 클라이언트가 SYN 플래그를 보내고, 서버는 SYN-ACK로 응답한다

**해설**: TCP 연결 설정 과정인 3-way 핸드셰이크는 다음과 같은 단계로 이루어집니다: 1) 클라이언트가 SYN(Synchronize) 플래그를 설정한 세그먼트를 서버에 전송하여 연결을 요청합니다. 2) 서버는 클라이언트의 SYN을 받고, SYN과 ACK(Acknowledgment) 플래그를 모두 설정한 세그먼트(SYN-ACK)로 응답합니다. 3) 클라이언트는 서버의 SYN-ACK를 받고, ACK 플래그를 설정한 세그먼트를 서버에 전송하여 연결 설정을 완료합니다. 이 과정을 통해 양방향 통신을 위한 연결이 설정되고, 양쪽의 초기 순서 번호가 동기화됩니다. FIN 플래그는 연결 종료 시에 사용되며, 3-way 핸드셰이크는 총 3개의 메시지가 교환됩니다.

#### 문제 4
**문제**: 다음 중 UDP를 사용하기에 가장 적합한 애플리케이션은?

**정답**: 실시간 비디오 스트리밍

**해설**: UDP는 실시간 비디오 스트리밍과 같이 속도와 지연 시간이 중요하고, 일부 데이터 손실이 허용되는 애플리케이션에 가장 적합합니다. 실시간 스트리밍에서는 패킷이 손실되더라도 재전송으로 인한 지연보다 빠른 전송이 더 중요한 경우가 많습니다. 또한 UDP는 연결 설정 과정이 없고 헤더 크기가 작아(8바이트) 오버헤드가 적어 빠른 데이터 전송이 가능합니다. 반면, 온라인 뱅킹 시스템, 이메일 전송, 파일 다운로드와 같이 데이터의 정확성과 완전성이 중요한 애플리케이션에는 신뢰성 있는 전송을 제공하는 TCP가 더 적합합니다.

#### 문제 5
**문제**: TCP 상태 중 연결이 설정되어 데이터를 주고받을 수 있는 상태는?

**정답**: ESTABLISHED

**해설**: TCP 연결에서 ESTABLISHED 상태는 연결이 완전히 설정되어 양방향으로 데이터를 주고받을 수 있는 상태를 의미합니다. 이 상태는 3-way 핸드셰이크가 성공적으로 완료된 후에 도달합니다. LISTEN 상태는 서버가 클라이언트의 연결 요청을 기다리는 상태이고, SYN_SENT 상태는 클라이언트가 SYN을 보내고 서버의 응답을 기다리는 상태입니다. TIME_WAIT 상태는 연결 종료 과정에서 마지막 ACK를 보낸 후, 지연된 패킷을 처리하기 위해 일정 시간 동안 대기하는 상태입니다. 실제 데이터 전송은 ESTABLISHED 상태에서만 이루어집니다.

#### 문제 6
**문제**: 다음 중 TCP와 UDP의 공통점은?

**정답**: 포트 번호를 사용하여 애플리케이션을 식별한다

**해설**: TCP와 UDP의 공통점은 포트 번호를 사용하여 애플리케이션을 식별한다는 것입니다. 두 프로토콜 모두 출발지 포트와 목적지 포트 필드를 헤더에 포함하고 있으며, 이를 통해 데이터가 어떤 애플리케이션에서 왔고 어떤 애플리케이션으로 가야 하는지 식별합니다. 이는 전송 계층의 중요한 기능인 다중화와 역다중화를 가능하게 합니다. 반면, 연결 설정 과정, 패킷 손실 시 재전송, 혼잡 제어 메커니즘은 TCP에만 있는 특징으로, UDP에는 이러한 기능이 없습니다. UDP는 단순히 데이터를 전송할 뿐, 연결 관리나 신뢰성 보장을 위한 메커니즘을 제공하지 않습니다.
